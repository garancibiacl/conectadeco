...
export default function Navbar() {
  ...
  const handleFavoritesClick = async (event) => {
    ...
    const result = await Swal.fire({
      icon: 'info',
      title: 'Crea tu cuenta para usar favoritos',
      text: 'Regístrate o inicia sesión para guardar tus productos favoritos.',
    ...
    <nav className="sticky top-0 z-50 border-b border-gray-100 bg-white/88 shadow-sm backdrop-blur-md">
      ...
      {menuOpen && (
        <div className="md:hidden border-t border-gray-100 bg-white px-4 py-3 flex flex-col gap-3 text-sm">
          ...
          <Link to="/favoritos" onClick={handleFavoritesClick} className="ui-button text-gray-700 hover:text-red-600">Favoritos</Link>
